...
function xbNavBar(title,clss,lclss){
   ...
   // Hamburger — opens the sidebar offcanvas on mobile (< lg).
   // Placed BEFORE the brand so it lands at the far left on mobile.
   // Bootstrap's navbar-expand-lg hides it on desktop automatically.
   ...
   // Nav items always visible — no collapse wrapper needed (just the user-menu icon)
   this.navList = document.createElement("ul");
   this.navList.className = "navbar-nav ms-auto " + (lclss || "");

   cont.appendChild(btn);       // far left on mobile
   cont.appendChild(brand);
   cont.appendChild(this.navList);  // ms-auto pushes to far right
