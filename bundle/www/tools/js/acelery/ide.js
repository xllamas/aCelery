import{html as R,render as Un,useState as da,useEffect as ua,useCallback as Kn,Button as Gn,applyTheme as fa,isDark as ma}from"acelery/ui.js";import{EDITOR_THEMES as zn}from"acelery/editor.js";import{html as U,useState as Fa}from"acelery/ui.js";import{useState as wa,useEffect as ga}from"acelery/ui.js";function ka(e){let t=String(e??"").replace(/^#\/?/,"").split("/").filter(Boolean).map(Sa);return{section:t[0]??"home",parts:t.slice(1)}}function Sa(e){try{return decodeURIComponent(e)}catch{return e}}function G(e){return"#/"+e.map(t=>encodeURIComponent(t)).join("/")}var be=()=>window.location.hash||"#/",Ue=new Set;function Ke(){for(let e of Ue)e(be())}var ht=!1;function Ca(){ht||(ht=!0,window.addEventListener("popstate",Ke),window.addEventListener("hashchange",Ke))}var X=null;function vt(e){return X=e,()=>{X===e&&(X=null)}}async function h(e,{replace:t=!1}={}){let a=G(e);a!==be()&&(X&&!await X()||(t?window.history.replaceState({from:window.history.state?.from??null},"",a):window.history.pushState({from:be()},"",a),Ke()))}async function yt(e){let t=G(e);if(window.history.state?.from===t){if(X&&!await X())return;window.history.back()}else await h(e,{replace:!0})}function bt(){Ca();let[e,t]=wa(be());return ga(()=>(Ue.add(t),t(be()),()=>Ue.delete(t)),[]),ka(e)}function wt(){new URLSearchParams(window.location.search).get("opt")==="apps"&&window.history.replaceState(null,"",window.location.pathname+G(["apps"]))}var Ge=null;function we(e){Ge=e}function Ae(e){return Ge!==e?!1:(Ge=null,!0)}import{html as w,useState as Ie,useEffect as xa,useCallback as Da,useContext as Ea,useRef as St,createContext as Ta,Modal as Pe,Dropdown as Le,Placeholder as gt,Alert as Aa,Toast as La,Button as eo}from"acelery/ui.js";var g=({name:e})=>w`<i class=${e} aria-hidden="true"></i>`,Pa="(min-width: 768px)",Ct="(min-width: 992px)";function ze(e){let t=()=>!!globalThis.matchMedia?.(e).matches,[a,n]=Ie(t);return xa(()=>{let o=globalThis.matchMedia?.(e);if(!o?.addEventListener)return;let i=()=>n(o.matches);return i(),o.addEventListener("change",i),()=>o.removeEventListener("change",i)},[e]),a}function H({icon:e,label:t,onClick:a,disabled:n,primary:o,className:i}){return w`
    ...
    <div class="ac-empty">
      ...
      ${a?w`<p>${a}</p>`:null}
      ${n??null}
    </div>`}function L({rows:e=3,grid:t=!1}){let a=(o,i)=>w`
    ...
    <${Aa} variant="danger" className="ac-error" dismissible=${!!t}
              onClose=${t}>
    ...
    <//>`}var xt=Ta(()=>{});function Dt({children:e}){let[t,a]=Ie([]),n=St(0),o=Da(r=>{let s=++n.current;a(c=>[...c.slice(-2),{id:s,text:r}])},[]),i=r=>a(s=>s.filter(c=>c.id!==r));return w`
    <${xt.Provider} value=${o}>
      ${e}
      <div class="ac-toasts">
        ${t.map(r=>w`
    ...
    <//>`}var O=()=>Ea(xt);function ce({show:e,onHide:t,title:a,children:n}){return w`
    <${Pe} show=${e} onHide=${t} centered dialogClassName="ac-sheet">
      ...
      <${Le} align="end" className="ac-over">
        ...
        <${Le.Menu} popperConfig=${{strategy:"fixed"}}>
          ${s.map(c=>w`
            <${Le.Item} as="button" key=${c.label} disabled=${!!c.disabled}
    ...
    </div>`}var kt=["#3d6b63","#1f6f8b","#6b4fa0","#a14a2b","#2e7d32","#8a5a00","#9c2f5e","#45617d"];function Ia(e){let t=0;for(let a of e)t=t*31+a.codePointAt(0)>>>0;return kt[t%kt.length]}function Je({name:e,icon:t}){let[a,n]=Ie(!1);return t&&!a?w`
      <div class="ac-tile">
        <img src=${t} alt="" onError=${()=>n(!0)} />
      </div>`:w`
    <div class="ac-tile" style=${{background:Ia(e)}} aria-hidden="true">
      ${([...e][0]??"?").toUpperCase()}
    ...
    </div>`}function D({title:e,subtitle:t,back:a,actions:n,subbar:o,fab:i,fill:r,children:s}){let[c,d]=Fa(!1);return U`
        ...
        ${c?W`
              <div class="ac-continue-card">
                ...
                <div class="ac-row-body">
                  ...
                  <div class="ac-row-meta">Database · ${fe(c.length)}</div>
                </div>
                <div class="ac-button-row">
    ...
    <${D} title=${W`
      ...
      <div class="ac-home">
    ...
    <//>`}import{html as _,useState as Ze,useEffect as Ka,useCallback as Ga,Button as za}from"acelery/ui.js";import{runApp as Ja,exportProject as Wa}from"acelery/export.js";import{html as _a,useState as Na,useCallback as Ua,Modal as Ce,Button as Kt}from"acelery/ui.js";function q(){let[e,t]=Na(null),a=Ua((i,{title:r="aCelery",danger:s=!1,confirmLabel:c="OK",cancelLabel:d="Cancel",dismissValue:l=!1}={})=>new Promise(u=>t({message:i,title:r,danger:s,confirmLabel:c,cancelLabel:d,dismissValue:l,resolve:u})),[]),n=i=>{e?.resolve(i),t(null)},o=e?_a`
        <${Ce} show onHide=${()=>n(e.dismissValue)} centered>
          ...
          <${Ce.Body}>${e.message}<//>
          <${Ce.Footer}>
            <${Kt} variant="outline-secondary" onClick=${()=>n(!1)}>
              ${e.cancelLabel}
            <//>
            <${Kt} variant=${e.danger?"danger":"primary"}
                       onClick=${()=>n(!0)}>
              ${e.confirmLabel}
            <//>
          <//>
    ...
    <div class="ac-search" role="search">
    ...
    </div>`}function tt(e,t){let a=t.trim().toLowerCase();return!a||e.name.toLowerCase().includes(a)||e.description.toLowerCase().includes(a)}async function xe(e,t){return e(`${t} and all of its files will be deleted. This cannot be undone.`,{title:`Delete ${t}?`,danger:!0,confirmLabel:"Delete"})}function Gt({settings:e,updateSettings:t}){let[a,n]=Ze(null),[o,i]=Ze(null),[r,s]=Ze(""),c=O(),{confirm:d,dialog:l}=q(),u=Ga(async()=>{try{n(await de())}catch(v){i(v),n([])}},[]);Ka(()=>{u()},[u]);async function y(v){if(await xe(d,v.name))try{await ue(v.name),e["recent.project"]===v.name&&t({"recent.project":"","recent.file":""}),c(`Deleted ${v.name}`),await u()}catch($){i($)}}let k;if(a===null)k=_`<${L} rows=${4} grid />`;else if(!a.length)k=_`
      ...
      ${v.length?_`
            <div class="ac-grid">
              ${v.map($=>_`
                <${Me} key=${$.name} project=${$} verb="Run"
                  onOpen=${()=>Ja($.name,$.name,!1)}
                  actions=${[{label:"Edit in Code",icon:"fa-solid fa-pen-to-square",onSelect:()=>h(["code",$.name])},Ht()&&{label:"Add to home screen",icon:"fa-solid fa-mobile-screen",onSelect:()=>ae({action:"addShortcut",app:$.name})},{label:"Export",icon:"fa-solid fa-file-export",onSelect:()=>Wa($.name)},{label:"Delete",icon:"fa-solid fa-trash",danger:!0,onSelect:()=>y($)}]} />`)}
            </div>`:_`<p class="text-body-secondary px-1">Nothing matches “${r}”.</p>`}`}return _`
    <${D} title="Apps">
      <${j} error=${o} onClose=${()=>i(null)} />
      ${k}
    <//>
    ${l}`}import{html as B,useState as De,useEffect as zt,useCallback as Qa,Button as at,Modal as Jt,Form as Va,Input as Wt,notEmpty as Ya}from"acelery/ui.js";import{runApp as Za,exportProject as Xa,importProject as en}from"acelery/export.js";function tn({show:e,scaffold:t,existing:a,onClose:n,onCreate:o}){let i=r=>{let s=(r??"").trim().toLowerCase();return a.some(c=>c.name.toLowerCase()===s)?"A project with that name already exists":!0};return B`
    <${ce} show=${e} onHide=${n} title="New project">
      <${Va} initial=${{name:"",description:""}} onSubmit=${o}>
        <${Jt.Body}>
          <${Wt} label="Name" name="name"
            placeholder="Letters and numbers, 16 max"
            autocapitalize="off" autocomplete="off" spellcheck=${!1}
            validate=${[Ya("A name is required"),r=>t.nameProblem(r)??!0,i]} />
          <${Wt} label="Description" name="description" as="textarea"
    ...
    <//>`}function Qt({settings:e,updateSettings:t}){let[a,n]=De(null),[o,i]=De(null),[r,s]=De(null),[c,d]=De(""),[l,u]=De(()=>Ae("new-project")),y=O(),{confirm:k,dialog:v}=q(),$=Qa(async()=>{try{n(await de())}catch(b){s(b),n([])}},[]);zt(()=>{$()},[$]),zt(()=>{je().then(i,s)},[]);async function m(b){u(!1);try{let C=await Bt(b);y(`Created ${C}`),h(["code",C,o.entry])}catch(C){s(C)}}async function S(b){if(await xe(k,b.name))try{await ue(b.name),e["recent.project"]===b.name&&t({"recent.project":"","recent.file":""}),y(`Deleted ${b.name}`),await $()}catch(C){s(C)}}let T;if(a===null)T=B`<${L} rows=${4} grid />`;else if(!a.length)T=B`
    ...
    <${ce} show=${e} onHide=${n} title=${`New file in ${t}`}>
      <${an} initial=${{name:"",type:".js"}} onSubmit=${o}>
        <${Yt.Body}>
          <${nn} label="Name" name="name" placeholder="File name without extension"
            autocapitalize="off" autocomplete="off" spellcheck=${!1}
            validate=${[rn("A name is required"),i=>/^[\w.-]+$/.test((i??"").trim())?!0:"Letters, numbers, dot, dash and underscore only"]} />
          <${on} label="Type" name="type" options=${[{label:"JavaScript",value:".js"},{label:"CSS",value:".css"}]} />
        <//>
        <${Yt.Footer}>
      ...
      <${D} title=${e} back=${["code"]}>
        ...
        <div class="ac-list">
          ${u.map(f=>x`
            <${J} key=${f.name} icon=${dn(f.name)} title=${f.name}
              meta=${`${nt[He(f.name)]??"File"} \xB7 ${fe(f.length)}`}
        ...
        <${M} icon="fa-solid fa-file-code" title="No files yet"
          action=${x`<${me} variant="primary" onClick=${()=>T(!0)}>
            New file
          <//>`}>
    ...
    ${pt}`}import{html as oe,useState as ot,useEffect as $n,useCallback as mn,Button as it,Modal as ea,Form as pn,Input as hn,notEmpty as vn}from"acelery/ui.js";import{openDB as yn,deleteDB as bn}from"acelery/sql.js";function wn({show:e,existing:t,onClose:a,onCreate:n}){return oe`
    <${ce} show=${e} onHide=${a} title="New database">
      <${pn} initial=${{name:""}} onSubmit=${n}>
        <${ea.Body}>
          <${hn} label="Name" name="name" placeholder="Database name without extension"
            autocapitalize="off" autocomplete="off" spellcheck=${!1}
            help="Saved as a SQLite file ending in .db."
            validate=${[vn("A name is required"),o=>/^[\w-]+$/.test((o??"").trim())?!0:"Letters, numbers, dash and underscore only",o=>t.some(i=>i.name===`${(o??"").trim()}.db`)?"A database with that name already exists":!0]} />
        <//>
        <${ea.Footer}>
    ...
    <//>`}function ta({settings:e,updateSettings:t}){let[a,n]=ot(null),[o,i]=ot(null),[r,s]=ot(()=>Ae("new-db")),c=O(),{confirm:d,dialog:l}=q(),u=mn(async()=>{try{n(await ke())}catch($){i($),n([])}},[]);$n(()=>{u()},[u]);async function y({name:$}){s(!1);let m=`${$.trim()}.db`;try{await(await yn(m)).close(),c(`Created ${m}`),h(["data",m])}catch(S){i(S)}}async function k($){if(await d(`${$} and every table in it will be deleted. This cannot be undone.`,{title:`Delete ${$}?`,danger:!0,confirmLabel:"Delete"}))try{await bn($),e["recent.db"]===$&&t({"recent.db":""}),c(`Deleted ${$}`),await u()}catch(S){i(S)}}let v;return a===null?v=oe`<${L} rows=${3} />`:a.length?v=oe`
      <div class="ac-list">
        ${a.map($=>oe`
          <${J} key=${$.name} icon="fa-solid fa-database" title=${$.name}
            meta=${[fe($.length),Ot($.modified)].filter(Boolean).join(" \xB7 ")}
            badge=${$.name==="acelery.db"?"settings":null}
            onOpen=${()=>h(["data",$.name])}
            actions=${[{label:"Delete",icon:"fa-solid fa-trash",danger:!0,onSelect:()=>k($.name)}]} />`)}
      </div>`:v=oe`
  ...
  name text not null
      ...
      <//>`;if(re==="table"&&he)return p`
      <${Pn} db=${o} dbName=${e} table=${he} structure=${Ee==="structure"}
        error=${ve} onError=${d}
    ...
    <${D} title=${e} back=${["data"]}
      ...
      subbar=${p`
        <${Fe} label="Database view" value=${se}
          ...
          onChange=${A=>h(A==="sql"?["data",e,"sql"]:["data",e],{replace:!0})} />`}>
      ${ve}
      ${o?se==="sql"?p`<${In} db=${o} sql=${l} setSql=${u}
                   result=${y} setResult=${k}
                   history=${v} setHistory=${$} />`:p`<${Ln} db=${o} dbName=${e} onError=${d}
                   onDrop=${C}
                   onCreate=${()=>{u(Tn),k(null),h(["data",e,"sql"],{replace:!0})}} />`:r==="loading"?p`<${L} rows=${3} />`:null}
    <//>
    ${T}`}function Ln({db:e,dbName:t,onError:a,onDrop:n,onCreate:o}){let[i,r]=N(null),[s,c]=N({}),d=gn(async()=>{try{let l=await e.select("select name from sqlite_master where type = ? order by name",["table"]);r(l.map(u=>u.name))}catch(l){a(l),r([])}},[e]);return Oe(()=>{d()},[d]),Oe(()=>{if(!i)return;let l=!0;return(async()=>{for(let u of i){if(!l)return;try{Re(u);let y=await e.select(`PRAGMA table_info(${u})`),k=await e.selectOne(`select count(*) as n from ${u}`);l&&c(v=>({...v,[u]:{columns:y.length,rows:k?.n??0}}))}catch{}}})(),()=>{l=!1}},[i]),i===null?p`<${L} rows=${3} />`:i.length?p`
    <div class="ac-list">
      ...
      <${M} icon="fa-solid fa-table" title="No tables yet"
        ...
        <${ie} autoClose="outside" align="end" className="ac-over">
          ...
          <${ie.Menu} className="ac-columns-menu" popperConfig=${{strategy:"fixed"}}>
            ${s.map(m=>p`
              <${Sn} key=${m.name} label=${m.name} className="mb-2"
                checked=${!d.has(m.name)}
                onChange=${S=>y(m.name,S)} />`)}
          <//>
        <//>`:null,$;return!e||s===null?$=p`<${L} rows=${4} />`:s.length?n?$=p`
        ...
        <${aa} hover size="sm">
          ...
          <tbody>
            ${s.map(m=>p`
              <tr key=${m.name}>
                <td>${m.name}</td>
                <td>${m.type||p`<span class="ac-null">none</span>`}</td>
                <td>${m.notnull?"yes":""}</td>
                <td>${m.dflt_value??p`<span class="ac-null">NULL</span>`}</td>
                <td>${m.pk?"primary":""}</td>
              </tr>`)}
          </tbody>
        <//>
      </div>`:$=p`
      <${Cn} key=${u.filter(m=>m.inList).map(m=>m.name).join("|")}
        db=${e} title=${a} table=${a} fields=${u} onError=${i} />`:$=p`
    ...
    <div class="ac-sql">
      ...
      <div class="ac-button-row">
        <${pe} variant="primary" onClick=${s} disabled=${!t.trim()}>
          <${g} name="fa-solid fa-play" /> Run
        <//>
        ...
        ${i.length?p`
              <${ie}>
                <${ie.Toggle} variant="outline-secondary">
                  <${g} name="fa-solid fa-clock-rotate-left" /> History
                <//>
                <${ie.Menu} popperConfig=${{strategy:"fixed"}}>
                  ${i.map(d=>p`
                    <${ie.Item} as="button" key=${d}
      ...
      ${c}
    </div>`}function Fn({result:e,onShowAll:t}){let{rows:a,ms:n,limit:o}=e;if(!a.length)return p`<div class="ac-results-meta" role="status">No rows · ${n} ms</div>`;let i=[...new Set(a.flatMap(s=>Object.keys(s)))],r=a.slice(0,o);return p`
        ...
        <div class="ac-list">
          <div class="ac-setting">
            <div class="ac-setting-label">
              ...
              <div class="ac-setting-help">
                ${i?"Follow the device, or always light or dark.":`${sa(o)} is always ${ra()?"dark":"light"}.`}
              </div>
            </div>
            <${Fe} role="radiogroup" label="Mode"
              value=${i?Bn():ra()?"dark":"light"}
              onChange=${c}
          ...
          <div class="ac-setting">
            ...
            <select id="settings-theme" class="form-select" value=${o}
              ...
              ${jn.map(l=>_e`<option key=${l} value=${l}>${sa(l)}</option>`)}
            </select>
          </div>
          <div class="ac-setting">
            <div class="ac-setting-label">
              ...
              <div class="ac-setting-help">
                ${r?`A ${On(r)?"dark":"light"} scheme, whatever the mode.`:"Light or dark, to match the app."}
              </div>
            </div>
            <select id="settings-editor" class="form-select" value=${r}
                    onChange=${l=>t({editortheme:l.currentTarget.value})}>
              ${Hn.map(l=>_e`<option key=${l.value} value=${l.value}>${l.label}</option>`)}
            </select>
          </div>
        </div>
      </section>
            ...
            <section class="ac-section" aria-labelledby="settings-device">
              ...
              <div class="ac-list">
                <${J} icon="fa-solid fa-wifi" title="Network access"
                  ...
                  onOpen=${()=>ae({action:"showNetworkAccess"})} />
                <div class="ac-setting">
                  <div class="ac-setting-label">
